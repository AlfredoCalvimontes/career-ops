...
| `data/seen-jobs.json` | Per-posting state keyed by canonical URL: rank (score + reason), eligibility/language gate verdicts, skill gaps vs `cv.md`, expiry. Written by `rank-pipeline.mjs` and `seen-jobs.mjs`; lets an unchanged posting reuse its rank instead of being re-scored. Also carries the role `tier` (from `config/profile.yml → role_tiers`) and stated `payMonthlyUsd` used by `role-tier.mjs shortlist`. Only adds knowledge: never deletes a pipeline row or changes a tracker status. A corrupt file is kept aside as `seen-jobs.json.corrupt-<ts>` and rebuilt. Safe to delete |
...
| `modes/linkedin.md` | LinkedIn profile audit/rewrite instructions |
| `modes/strategy.md` | Role-strategy instructions (which role families to target and why) |
