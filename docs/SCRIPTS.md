...
| `npm run handoff -- <strategy\|market\|practice\|outreach>` | `handoff.mjs` | Export one self-contained prompt for a cheaper AI from your own files: `modes/_brief.md` (plus `cv.md` with `--with-cv`) as FACTS with emails/phones redacted, and any `--input` files (job descriptions, a contact's profile) as fenced untrusted DATA. `market` needs 1–10 inputs, `practice` takes `--role/--company/--round`, `outreach` takes `--contact/--limit`. Never includes `story-bank.md`. Read-only, no network, no LLM; `--out` writes a file |
| `npm run tier -- <classify|board|shortlist|set-pay>` | `role-tier.mjs` | Rank roles by how much you want them: `classify "<title>" [--score N --pay USD]` gives T1/T2/T3 (from `config/profile.yml → role_tiers`) with the tier policy applied (T2 capped 4.0, T3 capped 3.5 and only worth it when pay beats your current job); `board` groups tracked applications by tier; `shortlist` groups ranked, not-yet-applied postings by tier; `set-pay` records a posting's monthly USD pay |
| `npm run upstream:watch [-- --base upgrades --json --out report.md]` | `scripts/upstream-triage.mjs` | What is new upstream: fetches `upstream/main`, lists commits your branch lacks (non-merge), groups them by Conventional Commit type (breaking / security / feat / fix / docs ...), flags those touching files your branch changed (likely conflicts). Read-only: never merges. `.github/workflows/upstream-watch.yml` runs it weekly and keeps one tracking issue up to date |
