 ...
 *       tier?:   { tier: 1|2|3, matched },   // role tier (lib/role-tiers.mjs)
 *       payMonthlyUsd?: number,      // stated pay, monthly USD
 *       expired?: { at, reason }
 *   } } }
 *
...
import { compareByTier } from './role-tiers.mjs';
...
export function sanitizeState(raw) {
  ...
  for (const [key, j] of Object.entries(raw.jobs)) {
    ...
    if (isObj(j.tier) && [1, 2, 3].includes(j.tier.tier)) job.tier = { tier: j.tier.tier, matched: j.tier.matched ? str(j.tier.matched, 120) : null };
    if (Number.isFinite(j.payMonthlyUsd) && j.payMonthlyUsd >= 0) job.payMonthlyUsd = Math.round(j.payMonthlyUsd);
...
/** Record the role tier (1|2|3) with the tier entry that matched; null clears it. */
export function setTier(state, key, tier, matched = null) {
  const job = state.jobs[key];
  if (!job) return false;
  if (tier == null) { delete job.tier; return true; }
  if (![1, 2, 3].includes(tier)) return false;
  job.tier = { tier, matched: matched ? str(matched, 120) : null };
  return true;
}

/** Record stated pay in monthly USD. */
export function setPay(state, key, monthlyUsd) {
  const job = state.jobs[key];
  const n = Number(monthlyUsd);
  if (!job || !Number.isFinite(n) || n < 0) return false;
  job.payMonthlyUsd = Math.round(n);
  return true;
}
 ...
 */
export function listJobs(state, { minRank, gate, expired, hasGaps, tier, sort } = {}) {
  return Object.entries(state.jobs)
    ...
    .filter((j) => (tier == null ? true : j.tier?.tier === tier))
    .sort(sort === 'tier'
      ? (a, b) => compareByTier({ tier: a.tier?.tier, score: a.rank?.score, payMonthlyUsd: a.payMonthlyUsd }, { tier: b.tier?.tier, score: b.rank?.score, payMonthlyUsd: b.payMonthlyUsd }) || a.key.localeCompare(b.key)
      : (a, b) => (b.rank?.score ?? -1) - (a.rank?.score ?? -1) || a.key.localeCompare(b.key));
}
